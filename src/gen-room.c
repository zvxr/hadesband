		...
		for (dx = 0; dx < xmax && *t; dx++, t++) {
			...
			case '8': {
				...
				if (randint0(100) < 80 || dun->persist) {
					...
					place_random_stairs(c, grid, dun->block_down_stairs);
				}
	...
	for (t = data, y = 0; y < v->hgt && *t; y++) {
		for (x = 0; x < v->wid && *t; x++, t++) {
			...
			case '>': {
				...
				if (dun->block_down_stairs || c->depth
						>= z_info->max_depth - 1) {
		...
		for (x = 0; x < v->wid && *t; x++, t++) {
			...
			/* Otherwise, analyze the symbol */
				...
				case '$': place_gold(c, grid, c->depth, ORIGIN_VAULT);break;
					/* Armour. */
				case ']': {
					int	tval = 0, temp = one_in_(3) ? randint1(10) : randint1(9);
					switch (temp) {
					...
					case 7: tval = TV_BELT; break;
					case 8: tval = TV_SOFT_ARMOR; break;
					case 9: tval = TV_HARD_ARMOR; break;
					case 10: tval = TV_DRAG_ARMOR; break;
					}
 ...
 * \param c the chunk the room is being built in
 * \param centre the room centre; out of chunk centre invokes find_space()
 * \param typ the name of the vault type (eg "Greater vault") to build
 * \return success
 */
...
bool build_large(struct chunk *c, struct loc centre, int rating)
{
	...
	case 2: {
		...
		if (randint0(100) < 80 || dun->persist) {
			...
			place_random_stairs(c, centre, dun->block_down_stairs);
		}
