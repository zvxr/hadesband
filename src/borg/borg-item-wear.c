...
bool borg_swap_rings(void)
{
    ...
    /* Cannot remove the Ring of Zeus */
...
bool borg_wear_rings(void)
{
    ...
    for (i = 0; i < z_info->pack_size; i++) {
        ...
        /* the Ring of Zeus would be awesome */
        if (item->one_ring)
            p = borg.power * 2;
...
bool borg_wear_stuff(void)
{
    ...
    for (i = 0; i < z_info->pack_size; i++) {
        ...
        if (slot != INVEN_LEFT
            ...
            /* Need to be careful not to put the Ring of Zeus onto
             * the Left Hand
             */
        ...
        if (randint0(100) == 10 || item->one_ring) {
            ...
            if (slot == INVEN_LEFT && borg_items[INVEN_LEFT].iqty
                ...
                for (ii = INVEN_RIGHT; ii <= INVEN_LEFT; ii++) {
                    ...
                    /* Does the Ring of Zeus need to be handled here? */
